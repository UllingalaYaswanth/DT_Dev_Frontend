...
function Sidebar() {
  ...
  return (
    <div className='w-16 md:w-64 fixed left-0 top-0 z-10 h-screen py-2 border-r pt-8 px-4 bg-[#4C2E46]  text-white rounded-md shadow-md'>
    <div className='items-center justify-center flex'>
      <h1 className='text-2xl  mt-5 hidden md:flex'>Operator Dashboard</h1>
    </div>
    <ul className='mt-14 space-y-4'>
      {
        SIDEBAR_LINKS.map((link, index) => (
    ...
    <div className='absolute bottom-5 left-5 cursor-pointer text-center py-2 px-2'>
      <p className='flex items-center space-x-2 text-md text-gray-800 font-semibold bg-indigo-100 hover:bg-red-400 hover:text-white py-2 md:px-12 rounded-md justify-center md:justify-start' onClick={handleLogout}>
        <LuLogOut className='flex' />
